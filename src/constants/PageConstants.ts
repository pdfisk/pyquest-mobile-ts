export class PageConstants {

    static pathBoard = 'board';
    static pathDelete = 'delete';
    static pathDetails = 'details';
    static pathEditor = 'editor';
    static pathHelp = 'help';
    static pathLogin = 'login';
    static pathNew = 'new';
    static pathProjects = 'projects';
    static pathPython = 'python';
    static pathPythonHelp = 'python_docs';
    static pathRegister = 'register';
    static pathRename = 'rename';
    static pathSelect = 'select';
    static pathStatus = 'status';
    static pathTopMenu = '';
    static pathTranscript = 'transcript';

    static routeBoard = '/' + this.pathBoard;
    static routeDelete = '/' + this.pathDelete;
    static routeDetails = '/' + this.pathDetails;
    static routeEditor = '/' + this.pathEditor;
    static routeHelp = '/' + this.pathHelp;
    static routeLogin = '/' + this.pathLogin;
    static routeNew = '/' + this.pathNew;
    static routeProjects = '/' + this.pathProjects;
    static routePython = '/' + this.pathPython;
    static routePythonHelp = '/' + this.pathPythonHelp;
    static routeRegister = '/' + this.pathRegister;
    static routeRename = '/' + this.pathRename;
    static routeSelect = '/' + this.pathSelect;
    static routeStatus = '/' + this.pathStatus;
    static routeTopMenu = '/' + this.pathTopMenu;
    static routeTranscript = '/' + this.pathTranscript;

    static subtitleBoard = 'Game board';
    static subtitleDelete = 'Delete project';
    static subtitleDetails = 'Project details';
    static subtitleEditor = 'Python editor';
    static subtitleForum = 'Support forum';
    static subtitleHelp = 'Help pages';
    static subtitleLogin = 'Login or register';
    static subtitleLogout = 'Logout';
    static subtitleNew = 'Create new project';
    static subtitleProjects = 'Python projects';
    static subtitlePythonHelp = 'Python help';
    static subtitleRegister = 'Register new account';
    static subtitleRename = 'Rename project';
    static subtitleSelect = 'Select projects';
    static subtitleStatus = 'Status information';
    static subtitleTopMenu = 'App pages menu';
    static subtitleTranscript = 'Text output';

    static titleBoard = 'Board';
    static titleDelete = 'Delete';
    static titleDetails = 'Details';
    static titleEditor = 'Editor';
    static titleHelp = 'Help';
    static titleLogin = 'Login';
    static titleLogout = 'Logout';
    static titleNew = 'New';
    static titleProjects = 'Projects';
    static titlePython = 'Python';
    static titlePythonHelp = 'Python Help';
    static titleRegister = 'Register';
    static titleRename = 'Rename';
    static titleSelect = 'Select';
    static titleStatus = 'Status';
    static titleSupport = 'Support';
    static titleTopMenu = 'Top Menu';
    static titleTranscript = 'Transcript';

}
